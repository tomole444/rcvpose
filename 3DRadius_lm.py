...
def new_main():
    dataset_path = "/home/thws_robotik/Documents/Leyh/6dpose/datasets/ownBuchRCV"
    z_max = 0

    print(dataset_path)
    
    pcd_load = o3d.io.read_point_cloud(os.path.join(dataset_path,"mesh.ply"))
    ...
    keypoints=np.load(os.path.join(dataset_path,"Outside9.npy"))
    points_count = 1
    ...
    for keypoint in keypoints:
        ...
        rootDict = dataset_path+"/" 
        GTDepthPath = rootDict+'FakeDepth/'
        if depthGeneration:
            saveDict = dataset_path+"/FakeDepth/"
        else:
            saveDict = dataset_path+"/Out_pt"+str(points_count)+"_dm/"    
        if(os.path.exists(saveDict)==False):
            os.mkdir(saveDict)
        points_count+=1
        iter_count = 0
        dataDict = rootDict + "depth/"
        for filename in os.listdir(dataDict):
            if filename.endswith(".dpt"):
                #and os.path.exists(saveDict+os.path.splitext(filename)[0][5:].zfill(6)+'.npy')==False
                #print(filename)
                #depth = np.load(GTDepthPath+os.path.splitext(filename)[0][5:].zfill(6)+'.npy')*1000
                realdepth = read_depth(dataDict+filename)
                mask = np.asarray(Image.open(dataset_path+"/mask/"+os.path.splitext(filename)[0][5:].zfill(4)+".png"), dtype=int)
                mask = mask[:,:,0]        
                #depth[np.where(mask==0)] = 0
                realdepth[np.where(mask==0)] = 0
                #plt.imshow(realdepth-depth*1000)
                #plt.show()
                Radius3DMap = np.zeros(mask.shape)
                RT = np.load(dataset_path+"/pose/pose"+os.path.splitext(filename)[0][5:]+".npy")
                #print(RT)
                #print(linemod_pose(rootDict,os.path.splitext(filename)[0][5:]))
                pixel_coor = np.argwhere(mask==255)
                xyz,y,x = rgbd_to_point_cloud(linemod_K, realdepth)
